namespace MeloStats.Services
{
    ...
    public class SpotifyApiService
    {
        ...
        public async Task<List<Playlist>> GetUsersPlaylistsAsync(ApplicationUser user)
        {
            var endpoint = "v1/me/playlists";
            var response = await FetchWebApi(user, endpoint, HttpMethod.Get);
            ...
            foreach(var item in items)
            {
                var spotifyPlaylistId = item["id"].ToString();
                var name = item["name"].ToString();
                var owner = item["owner"]["display_name"].ToString();

                var playlist = await _context.Playlists.FirstOrDefaultAsync(p => p.SpotifyPlaylistId ==  spotifyPlaylistId);
                if (playlist == null)
                {
                ...
                // calling the action that add the songs of the playlist in the database
                await getPlaylistSongs(spotifyPlaylistId);
            }

            return playlists;
        }
        public async Task getPlaylistSongs(string id)
        {
            var accessToken = await _authService.GetAccessTokenAsync();
            var client = new HttpClient();
            client.BaseAddress = new Uri(BaseUrl);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
            var response = await client.GetAsync($"playlists/{id}/tracks");
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception($"Failed to fetch playlist details. Status Code: {response.StatusCode}");
            }
            var responseBody = await response.Content.ReadAsStringAsync();
            var json = JObject.Parse(responseBody);
            var items = json["items"];
            var playlist = await _context.Playlists.FirstOrDefaultAsync(p => p.SpotifyPlaylistId == id);
            foreach (var item in items)
            {
                var spotifyTrackId = item["id"].ToString();
                var trackName = item["name"].ToString();
                var duration = int.Parse(item["duration_ms"].ToString()) / 1000;
                var track = await _context.Tracks.FirstOrDefaultAsync(t => t.SpotifyTrackId == spotifyTrackId);
                ...
                if (artist == null || artist.Popularity == 0)
                {
                    ...
                    var artistResponse = await client.GetAsync($"artists/{spotifyArtistId}");
                    if (!artistResponse.IsSuccessStatusCode)
                    {
                        throw new Exception($"Failed to fetch artist details. Status Code: {response.StatusCode}");
                    }
                    responseBody = await response.Content.ReadAsStringAsync();
                    json = JObject.Parse(responseBody);

                    var artistPopularity = int.Parse(json["popularity"].ToString());
                    var artistGenres = json["genres"].Select(g => g.ToString()).ToArray();
                    var artistImg = json["images"].FirstOrDefault()?["url"]?.ToString();
                    var genres = string.Join(", ", artistGenres);
                ...
                var playlistTrack = await _context.PlaylistTracks.FirstOrDefaultAsync(pt => pt.TrackId == track.Id && pt.PlaylistId == playlist.Id);
